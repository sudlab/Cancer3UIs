...
tx2gene<-read.delim(file='expression.dir/tx2gene.tsv', header = TRUE, sep = '\t', dec = '.')
db_artifacts<-read.delim(file='expression.dir/false_positives.txt', header = TRUE, sep = '\t', dec = '.')
...
IDs<-list.files(dir, pattern = "sf", recursive=TRUE)
fileno<-length(IDs)
...
txi.tr<-tximport(files, type="salmon", txOut=TRUE, tx2gene = tx2gene, dropInfReps = TRUE) #dropInfReps gets rid of this error: "Error: package_version(minfo$salmon_version) >= "0.8.0" is not TRUE"

#Gene-level summarization with txOut=FALSE
txi.sum<-tximport(files, type="salmon", txOut=FALSE, tx2gene = tx2gene, dropInfReps = TRUE)
...
write.table(tpm.sum, "expression.dir/gene.tpm.tsv", sep="\t", row.names=FALSE, quote=FALSE)
write.table(tpm.tr, "expression.dir/transcript.tpm.tsv", sep="\t", row.names=FALSE, quote=FALSE)
write.table(fract.expr, "expression.dir/fraction_expression.tsv", sep="\t", row.names=FALSE, quote=FALSE)
