...
```{r}
all_utrons <- read.delim("../../../../final_classifications/utron_beds.dir/CCLE-agg-agg.all_utrons.ids.gz", header=F)
novel_utrons <- read.delim("../../../../final_classifications/utron_beds.dir/CCLE-agg-agg.novel_utrons.ids.gz", header=F)
partnered_utrons <- read.delim("../../../../final_classifications/utron_beds.dir/CCLE-agg-agg.partnered_utrons.ids.gz", header=F)
non_cds <- read.delim("../../../../final_classifications/utron_beds.dir/CCLE-agg-agg.no_cds_utrons.ids.gz", header=F)
...
suplementary_data <- dds_result %>%
  ...
  relocate(gene_id)
write.table(suplementary_data, "Supplementary_data_9.tsv", sep="\t", row.names=F, quote=F)
...
read.delim("../../../../../HCT-NMD_KD/requant/rmats.dir/HCT116/RI.MATS.JC.txt") %>%
  ...
  distinct() %>%
  write.table("Supplmentary_data_10.tsv", sep="\t", row.names=F, quote=F)
...
read.delim("../../../../../HCT_CHIR/RMATS/outputs/RI.MATS.JC.txt") %>%
  dplyr::select(-ID, -ID.1) %>%
  distinct() %>%
  write.table("Supplmentary_data_11.tsv", sep="\t", row.names=F, quote=F)
