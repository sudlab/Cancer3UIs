...
```{r}
library(ggplot2)
non_cds <- read.delim("../../../all_TCGA/new_classification/utron_beds.dir/agg-agg-agg.no_cds.ids.gz", header=F)
...
plot_data %>% rename(nonPTC=non_CDS)%>%
ggplot() +
  aes(x=log2FoldChange, col=nonPTC) + 
  stat_ecdf() + 
  ...
  annotate("text", label=paste0("D=",round(test$statistic,3)), x=-5, y=1, hjust=0) + 
  annotate("text", label=paste0("p=",round(test$p.value, 12)), x=-5, y=0.9, hjust=0)
ggsave("ptc_vs_nonPTC.png", height=2.5, width=2.5, units="in")
```
...
ggplot(plot_data) +
  ...
  theme(legend.position = "top", legend.text = element_text(size=8),
        axis.text = element_text(size=8)) + ylab("") + xlab("log2FoldChange(siUPF1)")+ 
  annotate("text", label=paste0("D=",round(test$statistic,3)), x=-5, y=1, hjust=0) + 
  annotate("text", label=paste0("p=",round(test$p.value,5)), x=-5, y=0.9, hjust=0)
ggsave("over_vs_under_55.png", width=2.5, height=2.5, units="in")
```